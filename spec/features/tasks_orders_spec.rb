...
RSpec.feature "TasksOrders", type: :feature do
  ...
  let!(:first_task) { Task.create(title: 'first_title', description: 'first_desc', end_at: Time.now, status: 0) }
  let!(:new_task) { Task.create(title: 'new_title', description: 'new_desc', end_at: Time.now + 1.day, status: 1) }
